...
| **Peer contrast** rebuilt properly -- within-scenario building and room medians of margin and slope, plus rank and peer count, at 100% coverage | Rejected for the third time. In the single-horizon head it moved the timing screen by ±20 with no consistent sign; in the multi-horizon head it is clearly worse (1588 against 1485 at fifteen swaps, 1619 against 1460 at eighteen). The `HANDOVER.md` section 4 attempt failed as Wiener features. **Three independent constructions have now failed; treat this as closed.** |
| **The AFT's predicted log-time as a head feature** | Worse: 1509 against 1485 at fifteen swaps, 1494 against 1460 at eighteen. |
| **Both together** | Head PR-AUC rises to 0.3892 -- the best of any head measured -- and the *operating points get worse* (1554 against 1460). PR-AUC integrates over thresholds we never use; the swap counts the leaderboard charges are the only measure that decides anything here. |
| **A head trained on the strided grid** (88,013 cutoffs, 4.4x the scenario frame) | Worse: head PR-AUC 0.3451 against 0.3594, blend 0.3784 against 0.3872, and worse at every swap count. Combining the two grids does not help either. The extra rows are the same 82 dying devices, and the population mismatch costs more than the volume buys -- trap 2, from the other direction. |
...
| **The full-strength retrain** -- the passage model at stride 2 / 400 iterations instead of stride 4 / 250, listed as unexplored since V6 | **+57.36 end to end, 16/48 wins.** Doubling the training data produces an *equivalent ranker*, not a better one: on the deployment population the two blends score PR-AUC 0.3842 against 0.3879 and trade places across the operating points -- better at fifteen swaps and above, worse at twelve and below. The regression is the planner landing at 15.19 swaps instead of 14.65 and losing precision there. It also costs about a second per scenario more at inference and twice the training time. **The passage model is not data-starved.** One caveat for anyone revisiting: `GroupKFold` assigns buildings by row count, so changing the stride also moves the fold partition and the head is refitted on different splits. The isolation is imperfect; the shipped configuration is the one that is validated. |
